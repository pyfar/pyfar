...
def test_fractional_octave_filter_tolerance(
        exact_center_frequency, num_fractions, tolerance_class):
    """Test values returned by fractional_octave_filter_tolerance."""

    # load reference data
    filename = Path((
        "tests/references/fractional_octave_filter_tolerance_"
...
def test_fractional_octave_filter_tolerance_errors():
    """Test if all errors are raised as expected."""

    with pytest.raises(ValueError, match='The exact center frequency'):
        pf.constants.fractional_octave_filter_tolerance([1000], 1, 1)

    with pytest.raises(ValueError, match="num_fractions is 2"):
        pf.constants.fractional_octave_filter_tolerance(1000, 2, 1)

    with pytest.raises(ValueError, match="tolerance_class is 0"):
        pf.constants.fractional_octave_filter_tolerance(1000, 1, 0)
