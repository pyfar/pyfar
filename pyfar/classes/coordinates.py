...
class Coordinates():
    """
    Create Coordinates object with or without cartesian coordinate points.

    For initialization with non-cartesian points use
    :py:func:`from_spherical_colatitude`,
    ...
    ----------
    x : array like, number
        The x-coordinates. Note that the x-coordinates must be broadcastable to
        `y` and `z`.
    y : array like, number
        The y-coordinates. Note that the y-coordinates must be broadcastable to
        `x` and `z`.
    z : array like, number
        The z-coordinates. Note that the z-coordinates must be broadcastable to
        `x` and `y`.
    ...
    def _set_points(self, x, y, z):
        """
        Check input and store in private ``_x``, ``_y``, and ``_z`` properties.

        Parameters
        ----------
        x, y, z: number, array like
            The x, y, and z coordinates. All input data must be broadcastable
            to a common shape.
        """
        # cast to numpy array
