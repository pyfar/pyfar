    ...
    def cycle_lines(self, event):
        if event.key in ['*', ']', '[', '/', '7']:
            ...
            self._plot_signal = Signal(
                ...
                'time', self.signal.signal_type)
            self.axes = _hplt._plot_spectrogram_cb(
                self._plot_signal, ax=self.axes)
            self.figure.canvas.draw()
